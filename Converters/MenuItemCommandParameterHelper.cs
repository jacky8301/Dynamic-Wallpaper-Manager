...
namespace WallpaperEngine.Converters
{
    ...
    public static class MenuItemCommandParameterHelper
    {
        ...
        private static void OnCommandParameterChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            Log.Information("  d type: {DType}", d?.GetType().Name ?? "null");
            Log.Information("  e.OldValue type: {OldValueType}, e.NewValue type: {NewValueType}", e.OldValue?.GetType().Name ?? "null", e.NewValue?.GetType().Name ?? "null");
            if (d is MenuItem menuItem)
            {
                Log.Information("  MenuItem Header: {Header}", menuItem.Header);
                if (e.NewValue is MultiBinding binding)
                {
                    Log.Information("MenuItemCommandParameterHelper: Setting binding for MenuItem");
                    Log.Information("  Binding has {Count} bindings", binding.Bindings.Count);
                    for (int i = 0; i < binding.Bindings.Count; i++)
                    {
                        Log.Information("    Binding[{Index}]: {Binding}", i, binding.Bindings[i].ToString());
                    }
                ...
                {
                    Log.Warning("MenuItemCommandParameterHelper: e.NewValue is not MultiBinding, type: {TypeName}", e.NewValue.GetType().Name);
                }
            ...
            {
                Log.Warning("MenuItemCommandParameterHelper: d is not MenuItem, type: {TypeName}", d?.GetType().Name);
            }
        }
